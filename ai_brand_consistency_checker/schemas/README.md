# AI Brand Consistency Checker Schemas
...
class AiBrandConsistencyCheckerJobInput(BaseModel):
    model_config = ConfigDict(populate_by_name=True)

    request_id: str = Field(..., min_length=1)
    tool_id: Literal["ai_brand_consistency_checker"]
    args: dict[str, Any] = Field(..., alias="input")
    options: dict[str, Any]
    metadata: dict[str, Any] | None = None

class AiBrandConsistencyCheckerJobOutput(BaseModel):
